...
def train(env_id, N_itr, seed, policy, baselr, lrschedule, num_env, nsteps, log_path, save_interval, alg):
    if policy == 'lstm':
    ...
    if alg == 'a2c':
        learn_a2c(policy=policy_fn, env=env, seed=seed, N_itr=int(N_itr), lrschedule=lrschedule,
            ...
            save_path=log_path,
            lr=baselr     )#,load_path="./Data/a2cTest/a2c_1.pkl")
    elif alg == 'ppo2':
        learn_ppo2(policy=policy_fn, env=env, seed=seed,nsteps=nsteps,nminibatches=4,
            lam=0.95, gamma=0.99, noptepochs=4, log_interval=1,
            ent_coef=.01,
            lr=lambda f : f * baselr, #2.5e-4,
...
def main():
    ...
    parser.add_argument('--baselr', help='base learning rate', type=float, default=1e-3)
    parser.add_argument('--lrschedule', help='Learning rate schedule', choices=['constant', 'linear'], default='constant')
    parser.add_argument('--nsteps', help='N steps', type=int, default=128)
    ...
    log_path = "./Data/" #+args.alg+'_'+args.policy+'_'+args.env+'_'+str(int(args.N_itr))+"steps_"+args.lrschedule+"Schedule_1e2baseLR/"
    
    for arg in vars(args):
        if arg in ['N_itr','baselr']:
            log_path+= arg + str(int(getattr(args,arg))) + '_'
        else:
            log_path+= arg + str(getattr(args,arg)) + '_'
    log_path=log_path[:-1]
    log_path+='/'
    
    logger.configure(dir=log_path)
    train(args.env, N_itr=args.N_itr, seed=args.seed,
        policy=args.policy, baselr=args.baselr, lrschedule=args.lrschedule, num_env=16, nsteps=args.nsteps, log_path=log_path, save_interval=args.save_interval,
        alg=args.alg)
