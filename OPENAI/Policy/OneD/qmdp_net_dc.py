...
class PlannerNet(object):
    def __init__(self, name, qmdp_param):
        ...
        # self.Qsa = create_param(tf.constant_initializer(0.1), (self.num_state*self.num_action,), name="Qsa", trainable=True, regularizable=False)
        initializer = tf.truncated_normal_initializer(mean=0.0, stddev=1.0, dtype=tf.float32)
        self.Qsa = create_param(initializer, (self.num_state*self.num_action,), name="Qsa", trainable=True, regularizable=False)

    def VI(self,n_batches):

        Qsa = self.Qsa
        Qsa = tf.tile(tf.reshape(Qsa, (1,self.num_state*self.num_action)),(n_batches,1))
        Q = self.f_R.step(Qsa)
...
class FilterNet(object):
    ...
    def beliefupdate(self, local_obs, actions, ms, b):
        ...
        # return local_obs, b
        return local_obs, b, w_O, Z_o, b_prime_a, b, b_old
...
class F_T_filter(object):
    def __init__(self, num_state, num_action,name):
        self.num_state = num_state
        self.num_action = num_action
        self.fclayers = FcLayers(num_state, np.array([[num_state*num_action, 'tanh']]), names=name+"-T_fc")
    def step(self, input):
        return tf.reshape(self.fclayers.step(input),[-1,self.num_state,self.num_action])
...
class F_O(object):
    def __init__(self, obs_len, num_obs,name):
        ...
        self.fclayers = FcLayers(obs_len, np.array([[num_obs, 'tanh']]), names=name+"-O_fc")
