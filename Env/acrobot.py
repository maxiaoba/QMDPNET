...
class AcrobotEnv(Env):
    ...
    **REFERENCE:**
    ...
    def __init__(self,PO=True):
        self.viewer = None
        if PO:
            self.high = np.array([1.0, 1.0, 1.0, 1.0])
        else:
            self.high = np.array([1.0, 1.0, 1.0, 1.0, self.MAX_VEL_1, self.MAX_VEL_2])
        self.low = -self.high
        self.num_action = self.actions_num
        self.state = None
        self.PO = PO
    ...
    def _get_ob(self):
        s = self.state
        if  self.PO:
            return np.array([cos(s[0]), np.sin(s[0]), cos(s[1]), sin(s[1])]) #just gave the angle, not speed, a POMDP
        else:
