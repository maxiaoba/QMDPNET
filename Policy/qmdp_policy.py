...
class QMDPPolicy(StochasticPolicy, LayersPowered, Serializable):
    def __init__(
            ...
            qmdp_param=None,
            # N=10, M=10, Pmove_succ=1.0, Pobs_succ=1.0,
    ):
        """
    ...
    def reset(self, map, goal, b0, dones=None):
        if dones is None:
            dones = [True]
        ...
        # sess.run(tf.assign(self.prob_network._l_gru.h0,h0))
        # sess.run(tf.assign(self.prob_network._l_gru.map,map))
        # sess.run(tf.assign(self.prob_network._l_gru.goal,goal))
        ...
        self.prob_network._l_output_flat.h0.load(h0, sess)
        self.prob_network._l_output_flat.map.load(map, sess)
        self.prob_network._l_output_flat.goal.load(goal, sess)
